...
% Define folder paths for the images
folderWithStone = 'C:\Users\tejas\Documents\MATLAB\Image With Stone';
folderWithoutStone = 'C:\Users\tejas\Documents\MATLAB\Image Without Stone';

% Create a custom menu for image selection with increased window size
choice = listdlg('PromptString', 'Choose an image type:', ...
                 'SelectionMode', 'single', ...
                 'ListString', {'Image with Stone', 'Image without Stone'}, ...
                 'ListSize', [400, 200]); % Adjust the ListSize as needed
...
% Select folder based on user's choice
if choice == 1
    folderPath = folderWithStone;
else
    folderPath = folderWithoutStone;
end

% Prompt user to select the image file from the chosen folder
[filename, pathname] = uigetfile({'*.*', 'All Files'}, 'Select Image', folderPath);
